...
class Master_i(TokenProcessor__POA.Master):
    def process_text(self, opt, arg):
        ...
        #     results = proc_pool.starmap(process_shard, shards.items())
        with ThreadPoolExecutor(max_workers=cpu_count()) as executor:
            # Submitting tasks to the thread pool using a list comprehension
            futures = [executor.submit(process_shard, shard_id, shard) for shard_id, shard in shards.items()]

            # Collecting the results as the futures complete
            results = [future.result() for future in as_completed(futures)]
        # shard_id, idmap, freqmap, embeddings_map = process_shard(shard_id, shard)
...
def spawn_worker(worker_id):
    ...
    instance_id = "i-0f1b66547267d7f21"
